...
/* ---- Node ID banner (always visible at top) ---- */
static lv_obj_t *s_node_banner = NULL;
static lv_obj_t *s_node_status = NULL;

static void create_node_banner(lv_obj_t *tile)
{
    /* Large node ID number — unmissable */
    char buf[32];
    snprintf(buf, sizeof(buf), "N%d", g_nvs_config.node_id);

    /* Cyan background bar behind node ID for high visibility */
    lv_obj_t *banner_bg = lv_obj_create(tile);
    lv_obj_set_size(banner_bg, 360, 70);
    lv_obj_align(banner_bg, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_set_style_bg_color(banner_bg, lv_color_make(0x00, 0x20, 0x33), 0);
    lv_obj_set_style_bg_opa(banner_bg, LV_OPA_COVER, 0);
    lv_obj_set_style_border_color(banner_bg, COLOR_CYAN, 0);
    lv_obj_set_style_border_width(banner_bg, 2, 0);
    lv_obj_set_style_border_side(banner_bg, LV_BORDER_SIDE_BOTTOM, 0);
    lv_obj_set_style_pad_all(banner_bg, 0, 0);

    s_node_banner = lv_label_create(banner_bg);
    lv_label_set_text(s_node_banner, buf);
    lv_obj_set_style_text_font(s_node_banner, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_color(s_node_banner, COLOR_CYAN, 0);
    lv_obj_set_style_text_letter_space(s_node_banner, 4, 0);
    lv_obj_align(s_node_banner, LV_ALIGN_LEFT_MID, 12, -8);

    /* Status line */
    s_node_status = lv_label_create(banner_bg);
    lv_label_set_text(s_node_status, "WiFi DensePose  ACTIVE");
    lv_obj_set_style_text_font(s_node_status, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_color(s_node_status, COLOR_GREEN, 0);
    lv_obj_align(s_node_status, LV_ALIGN_LEFT_MID, 12, 14);
}

/* ---- View 0: Dashboard ---- */
static void create_dashboard(lv_obj_t *tile)
{
    create_node_banner(tile);

    /* CSI amplitude chart */
    s_dash_chart = lv_chart_create(tile);
    lv_obj_set_size(s_dash_chart, 400, 100);
    lv_obj_align(s_dash_chart, LV_ALIGN_TOP_LEFT, 0, 84);
