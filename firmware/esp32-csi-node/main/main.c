...
void app_main(void)
{
    ...
    if (g_nvs_config.wifi_ssid[0] == '\0') {
        /* No WiFi SSID configured — enter BLE provisioning mode.
         * ble_prov_run() blocks until credentials are received and WiFi
         * connects.  It also saves the credentials to NVS and reloads
         * g_nvs_config, so we can proceed directly to CSI operation. */
        ESP_LOGI(TAG, "No WiFi SSID configured — entering BLE provisioning mode");
        esp_err_t prov_ret = ble_prov_run();
        if (prov_ret != ESP_OK) {
            ESP_LOGE(TAG, "BLE provisioning failed: %s", esp_err_to_name(prov_ret));
            /* Board will restart on prov failure inside ble_prov_run(). */
        }
        /* Register the main WiFi event handler for reconnection during
         * normal CSI operation.  ble_prov_run() unregistered its own
         * handlers, so we need the retry logic from event_handler(). */
        s_wifi_event_group = xEventGroupCreate();
        xEventGroupSetBits(s_wifi_event_group, WIFI_CONNECTED_BIT);
        ...
        /* SSID already configured — connect directly. */
        wifi_init_sta();
    }
#else
    ...
    /* Display already started early (before WiFi) — see above. */
