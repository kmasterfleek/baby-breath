...
pub fn raw_classify(score: f64) -> String {
    if score > 0.50 { "active".into() }
    else if score > 0.25 { "present_moving".into() }
    else if score > 0.06 { "present_still".into() }
    else { "absent".into() }
}
...
pub fn smooth_and_classify(state: &mut AppStateInner, raw: &mut ClassificationInfo, raw_motion: f64) {
    ...
    raw.presence = sm > 0.05;
    raw.confidence = (0.3 + sm.min(0.7) * 1.0).clamp(0.0, 1.0);
}
