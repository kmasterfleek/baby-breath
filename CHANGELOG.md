...
- **Workspace test suite: 1,065 passed, 1 failed (pre-existing).** `wifi-densepose-signal::ruvsense::field_model::tests::test_estimate_occupancy_noise_only` is gated behind `#[cfg(feature = "eigenvalue")]`, but `wifi-densepose-signal` has `default = ["eigenvalue"]` and downstream crates depend on it with default features, so `cargo test --workspace --no-default-features` still unifies the feature on and the test runs — and fails (eigenvalue estimator reports 4 persons on noise-only frames). Untouched by today's changes; the product UI no longer displays person count. To be fixed in the estimator, not the test.
...
### Changed
- Firmware `version.txt` bumped `0.6.0` → `0.6.1`.
- Refreshed `firmware/esp32-csi-node/release_bins/` 8MB set (`bootloader.bin`, `partition-table.bin`, `ota_data_initial.bin`, `esp32-csi-node.bin`) with the corrected binaries.

### Known Issues
- The 4MB SuperMini variant (`esp32-csi-node-4mb.bin`) still carries the old `node_id` bug — not yet rebuilt (the current ~1.3MB app may not fit the 4MB app partition without config tuning) and needs validation on 4MB hardware.
